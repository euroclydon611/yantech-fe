...
const EditUser = ({ open, onClose, itemData, refetch }: EditUserProps) => {
  const [privilege_ids, setPrivilege_ids] = useState<string[]>([]);

  const { data: userPrivileges } = useUserPrivilegesQuery({}, { skip: !open });
  ...
  const [userUpdate, { data, isSuccess, error, isLoading, reset }] =
    useUserUpdateMutation();

  const formik = useFormik({
    initialValues: {
      firstname: (itemData?.firstname as string) || "",
      other_names: (itemData?.other_names as string) || "",
      lastname: (itemData?.lastname as string) || "",
  ...
  useEffect(() => {
    if (open && itemData) {
      const ids = (itemData.privileges || []).map((p: any) => 
        typeof p === "string" ? p : p._id
      );
      setPrivilege_ids(ids);
    }
    if (!open) {
      formik.resetForm();
      setPrivilege_ids([]);
  ...
  const handleSelectAllCategory = (category: string, checked: boolean) => {
    ...
    if (checked) {
      setPrivilege_ids((prev) => Array.from(new Set([...prev, ...categoryIds])));
    } else {
      setPrivilege_ids((prev) => prev.filter((id) => !categoryIds.includes(id)));
    }
  };
