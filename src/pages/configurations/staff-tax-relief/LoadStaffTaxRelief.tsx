...
import { useBulkLoadStaffTaxReliefsMutation } from "../../../redux/features/configurations/staffTaxReliefApi";
import { styles } from "../../../styles";
import Swal from "sweetalert2";
import * as XLSX from "xlsx";

interface LoadStaffTaxReliefProps {
  open: boolean;
  onClose: () => void;
  refetch: () => void;
}

const LoadStaffTaxRelief = ({ open, onClose, refetch }: LoadStaffTaxReliefProps) => {
  const [file, setFile] = useState<File | null>(null);
  const [failedRows, setFailedRows] = useState<any[]>([]);
  const [bulkLoad, { isLoading, isSuccess, error, data }] = useBulkLoadStaffTaxReliefsMutation();
  ...
  const handleUpload = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to process this bulk staff tax relief file?",
      icon: "question",
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const fileInput = document.getElementById("relief-upload-input") as HTMLInputElement;
  ...
  const exportFailedRecords = () => {
    ...
    XLSX.writeFile(wb, `failed_staff_tax_reliefs_${new Date().getTime()}.xlsx`);
  };

  const handleDownloadSample = () => {
    const data = [
      {
        'RELIEF NAME': 'Marriage Relief',
        'STAFF ID': '0001',
        'AMOUNT': 100.00,
        'STATUS': 'active'
      },
      {
        'RELIEF NAME': 'Child Education Relief',
        'STAFF ID': '0002',
        'AMOUNT': 200.00,
        'STATUS': 'active'
      },
      {
        'RELIEF NAME': 'Old Age Relief',
        'STAFF ID': '0003',
        'AMOUNT': 150.00,
        'STATUS': 'inactive'
      }
    ...
    XLSX.writeFile(wb, "Staff_Tax_Relief_Sample.xlsx");
  };

  return (
    <Drawer
      title="Bulk Load Staff Tax Relief"
      onClose={onClose}
      open={open}
      width={800}
      maskClosable={false}
          ...
          description={
            <div className="text-xs">
              ...
              <p className="font-mono mt-1 font-bold italic text-blue-700">
                RELIEF NAME, STAFF ID, AMOUNT, STATUS
              </p>
              <p className="mt-2 text-gray-500 italic">* STATUS is optional (defaults to active)</p>
            </div>
          }
          type="info"
          showIcon
          ...
          <div className="mt-8">
            ...
            <Table
              ...
              columns={[
                ...
                { title: "Relief Name", dataIndex: "RELIEF NAME", key: "relief_name" },
                { 
                  title: "Error Reason", 
                  dataIndex: "ERROR", 
                  key: "error",
...
export default LoadStaffTaxRelief;
