...
const EditPME = ({ open, onClose, itemData, refetch }: EditPMEProps) => {
  const { data: singlePmeData, isLoading: isPmeLoading } = usePmeSingleQuery(itemData?._id, {
    skip: !open || !itemData?._id,
  });

  const pmeToUse = singlePmeData?.data || itemData;

  const [allowance_perc_allowances_ids, setAllowance_perc_allowances_ids] =
    useState(pmeToUse?.allowance_perc_allowances_ids || []);
  const [deduction_perc_allowances_ids, setDeduction_perc_allowances_ids] =
    useState(pmeToUse?.deduction_perc_allowances_ids || []);

  const [employees, setEmployees] = useState(pmeToUse?.users || []);
  ...
  const [pmeUpdate, { data, isSuccess, isLoading, error }] =
    usePmeUpdateMutation();
  ...
  const formik = useFormik({
    initialValues: {
      name: (pmeToUse?.name as string) || "",
      level: pmeToUse?.level
        ? (capitalizeFirstLetter(pmeToUse?.level) as string)
        : "",
      type: (pmeToUse?.type as string) || "",
      from_date: (pmeToUse?.from_date?.substring(0, 10) as string) || "",
      to_date: (pmeToUse?.to_date?.substring(0, 10) as string) || "",
      allowance_uom: (pmeToUse?.allowance_uom as string) || "",
      allowance_amount: (pmeToUse?.allowance_amount as string) || "",
      allowance_percentage: (pmeToUse?.allowance_percentage as string) || "",
      allowance_percentage_to:
        (pmeToUse?.allowance_percentage_to as string) || "",
      allowance_category: (pmeToUse?.allowance_category as string) || "taxable",
      // allowance_perc_allowances_ids: [],
      deduction_uom: (pmeToUse?.deduction_uom as string) || "",
      deduction_amount: (pmeToUse?.deduction_amount as string) || "",
      deduction_percentage: (pmeToUse?.deduction_percentage as string) || "",
      deduction_percentage_to:
        (pmeToUse?.deduction_percentage_to as string) || "",
      deduction_category: (pmeToUse?.deduction_category as string) || "",
      // deduction_perc_allowances_ids: "",
      bonus_amount: (pmeToUse?.bonus_amount as string) || "",
    },
    ...
    onSubmit: async ({
      ...
      await pmeUpdate({
        ...
        from_date,
        to_date,
        users: employees.filter((id: any) => id !== ""),
        allowance_percentage,
        allowance_percentage_to,
        allowance_perc_allowances_ids,
        allowance_category,
  ...
  useEffect(() => {
    if (open && pmeToUse) {
      setAllowance_perc_allowances_ids(
        pmeToUse?.allowance_perc_allowances_ids || []
      );
      setDeduction_perc_allowances_ids(
        pmeToUse?.deduction_perc_allowances_ids || []
      );
      setEmployees(pmeToUse?.users || []);
    }
  }, [open, singlePmeData, itemData]);
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              <select
                ...
                value={values.level}
                onChange={handleChange}
                className={`${styles.input} disabled:bg-gray-200`}
                disabled
              >
                <option value="">------</option>
