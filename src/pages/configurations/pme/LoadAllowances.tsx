...
import { useBulkStoreAllowancesMutation } from "../../../redux/features/configurations/pmeApi";
import { styles } from "../../../styles";
import Swal from "sweetalert2";
import * as XLSX from "xlsx";

interface LoadAllowancesProps {
  open: boolean;
  onClose: () => void;
  refetch: () => void;
}

const LoadAllowances = ({ open, onClose, refetch }: LoadAllowancesProps) => {
  const [file, setFile] = useState<File | null>(null);
  const [failedRows, setFailedRows] = useState<any[]>([]);
  const [bulkLoad, { isLoading, isSuccess, error, data }] = useBulkStoreAllowancesMutation();
  ...
  const handleUpload = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to process this bulk allowances file?",
      icon: "question",
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const fileInput = document.getElementById("allowance-upload-input") as HTMLInputElement;
  ...
  const exportFailedRecords = () => {
    ...
    XLSX.writeFile(wb, `failed_allowances_${new Date().getTime()}.xlsx`);
  };
  ...
  const handleDownloadSample = () => {
    ...
    XLSX.writeFile(wb, "Allowances_Sample.xlsx");
  };

  return (
    <Drawer
      title="Bulk Load Allowances"
      onClose={onClose}
      open={open}
      width={800}
      maskClosable={false}
