import { Drawer, Button, Table, Alert, Space } from "antd";
import { useState, useEffect } from "react";
import { AiOutlineUpload, AiOutlineDownload } from "react-icons/ai";
import { useBulkStoreDeductionsMutation } from "../../../redux/features/configurations/pmeApi";
import { styles } from "../../../styles";
import Swal from "sweetalert2";
import * as XLSX from "xlsx";

interface LoadDeductionsProps {
  open: boolean;
  onClose: () => void;
  refetch: () => void;
}

const LoadDeductions = ({ open, onClose, refetch }: LoadDeductionsProps) => {
  const [file, setFile] = useState<File | null>(null);
  const [failedRows, setFailedRows] = useState<any[]>([]);
  const [bulkLoad, { isLoading, isSuccess, error, data }] = useBulkStoreDeductionsMutation();
  ...
  const handleUpload = async () => {
    ...
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to process this bulk deductions file?",
      icon: "question",
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        const fileInput = document.getElementById("deduction-upload-input") as HTMLInputElement;
  ...
  const exportFailedRecords = () => {
    ...
    XLSX.writeFile(wb, `failed_deductions_${new Date().getTime()}.xlsx`);
  };
  ...
  const handleDownloadSample = () => {
    ...
    XLSX.writeFile(wb, "Deductions_Sample.xlsx");
  };

  return (
    <Drawer
      title="Bulk Load Deductions"
      onClose={onClose}
      open={open}
      width={800}
      maskClosable={false}
        ...
        <Alert
          ...
          description={
            <div className="text-xs">
              ...
              <p className="font-mono mt-1 font-bold italic text-blue-700">NAME, STAFF ID, AMOUNT</p>
            </div>
          }
          type="info"
          showIcon
