...
import { useLoanReportQuery } from "../../redux/features/reports/loanReportApi";
...
const LoanReport = () => {
  ...
  month = monthNames[parseInt(monthNo) - 1] || "January";

  const handleDateChange = (e: any) => {
    setSelectedMonth(e.target.value);
  };

  const { data: loanReportData, isLoading } = useLoanReportQuery({
    page,
    limit,
    searchTerm,
    sortField,
    ...
    year,
    forecast,
    currency: selectedCurrencyData?.code || "GHS",
  });
  ...
  return (
    <>
      <PageLayout
        title="Loan Report"
        breadcrumbs={[{ label: "Payroll" }, { label: "Reports" }, { label: "Loan" }]}
      />
      <div className="p-4">
        ...
        <div className="flex justify-between items-center w-full p-4 bg-white rounded-md">
          ...
          <div className="flex gap-x-4">
            <button
              ...
              onClick={() =>
                exportData(
                  `reports/export-loan?forecast=${forecast}&year=${year}&month=${month}&currency=${
                    selectedCurrencyData?.code || "GHS"
                  }`,
                  `Consolidated_Loan_Report_${month}_${year}_${
                    selectedCurrencyData?.code || "GHS"
                  }`
                )
              }
            ...
            <button
              ...
              onClick={() =>
                exportZipData(
                  `reports/export-loan-zip?forecast=${forecast}&year=${year}&month=${month}&currency=${
                    selectedCurrencyData?.code || "GHS"
                  }`,
                  `Lender_Loan_Reports_${month}_${year}_${
                    selectedCurrencyData?.code || "GHS"
                  }`
                )
              }
            >
              <MdDownload size={18} />
              <span className="max-md:hidden flex">Lender-wise (ZIP)</span>
            </button>
          </div>
        </div>
        <div>
          <div className="bg-white p-4">
            ...
            <div className="table-wrapper bg-white rounded-sm shadow-sm">
              <Table
                sticky
                dataSource={loanReportData?.data || []}
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    dataIndex: "outstanding_balance_before",
                    key: "balance_before",
                    width: 120,
                    align: "right",
                    sorter: true,
                    onHeaderCell: () => ({
                      onClick: () => handleSort("outstanding_balance_before"),
                      style: { cursor: "pointer" },
                    }),
                  ...
                  {
                    title: "Balance After",
                    dataIndex: "outstanding_balance_after",
                    key: "balance_after",
                    width: 120,
                    align: "right",
                    sorter: true,
                    onHeaderCell: () => ({
                      onClick: () => handleSort("outstanding_balance_after"),
                      style: { cursor: "pointer" },
                    }),
                    render: (val) => formatNumber(val) || "0.00",
                    onCell: () => ({
            ...
            <div className="flex mt-4 justify-end">
              <Pagination
                current={page}
                pageSize={limit}
                total={loanReportData && loanReportData.totalCount}
