...
const BankReport = () => {
  ...
  const { data: bankReportData, isLoading } = useBankReportQuery({
    page,
    limit,
    searchTerm,
    sortField,
  ...
  return (
    <>
      <PageLayout
        title="Bank Report"
        breadcrumbs={[{ label: "Payroll" }, { label: "Reports" }, { label: "Bank" }]}
      />
      <div className="p-4">
        ...
        <div className="flex justify-between items-center w-full p-4 bg-white rounded-md">
          ...
          <div className="flex gap-x-4">
            <button
              ...
              onClick={() =>
                exportData(
                  `reports/export-consolidated-net-pay?forecast=${forecast}&year=${year}&month=${month}&currency=${selectedCurrencyData.code}`,
                  `Consolidated_Bank_report_${month}_${year}_${selectedCurrencyData.code}`
                )
              }
            >
              <MdDownload size={18} />
              <span className="max-md:hidden flex">
                Netpay Summary (Excel)
              </span>
            </button>
            <button
              className={styles.export_btn}
              onClick={() =>
                exportZipData(
                  `reports/export-net-pay-zip?forecast=${forecast}&year=${year}&month=${month}&currency=${selectedCurrencyData.code}`,
                  `Bank_report_${month}_${year}_${selectedCurrencyData.code}`
                )
              }
            >
              <MdDownload size={18} />
