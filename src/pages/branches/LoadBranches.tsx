...
import { useLoadBranchesDataMutation } from "../../redux/features/bank/branchApi";
...
const LoadBranches = ({ setOpen, refetch }: any) => {
  const [loadBranchesData, { data, isSuccess, isLoading, error }] =
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      const failedMessage = failed
        ?.map(
          (item: any) =>
            `${item["Branch Name"] || "Unknown Branch"} - ${item["ERROR"] || item["error"] || "Unknown Error"}`
        )
        .join("<br><hr>");
