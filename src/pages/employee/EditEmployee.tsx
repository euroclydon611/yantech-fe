...
import { useTaxReliefListQuery } from "../../redux/features/configurations/taxApi";
...
const schema = Yup.object().shape({
  ...
  hire_start_date: Yup.date()
    .nullable()
    .transform((value, originalValue) => (originalValue === "" ? null : value))
    .typeError("Invalid date format"),

  assumption_date: Yup.date()
    .nullable()
    .transform((value, originalValue) => (originalValue === "" ? null : value))
    .typeError("Invalid date format"),
...
const EditEmployee = ({
  ...
  if (!isDrawer) PageTitle("Edit Employee(s) | HR");
  ...
  const { data: taxReliefData } = useTaxReliefListQuery(
    {},
    { refetchOnMountOrArgChange: true }
  );
  ...
  const { data: employeeInfo, refetch: refetchEmployee } =
    useGetEmployeeByIdQuery(
      {
        employeeId,
      },
  ...
  const handleNextButton = () => {
    setEmployeeId(employeeInfo?.next);
  };

  const handlePrevButton = () => {
    setEmployeeId(employeeInfo?.prev);
  };
  ...
  const [employeeUpdate, { data, isSuccess, isLoading, error }] =
    useEmployeeUpdateMutation();
  ...
  useEffect(() => {
    if (employeeInfo && employeeInfo.employee) {
      ...
      formik.setValues({
        ...
        date_of_birth: formatDateForInput(date_of_birth),
        gender: gender || "",
        ...
        ssnit_tier2_number: ssnit_tier2_number || "",
        ssnit_tier3_number: ssnit_tier3_number || "",
        bank_id: bank_id || "",
        ...
        grade_id: grade_id || "",
        notch: notch || "",
        grade_incremental_ids: grade_incremental_ids || [],
        department_id: department_id || "",
        unit_id: unit_id || "",
        hire_start_date: formatDateForInput(hire_start_date),
        hire_end_date: formatDateForInput(hire_end_date),
        assumption_date: formatDateForInput(assumption_date),
        appointment_letter: appointment_letter || "",
        address: address || "",
        house_number: house_number || "",
        ghana_post: ghana_post || "",
        personnal_pme_ids: personnal_pme_ids || [],
        status: status_id || "",
        wage_type: wage_type || "",
        basic_salary: basic_salary || "0",
        hourly_rate: hourly_rate || "0",
        subcontractor: subcontractor ? true : false,
        consultant: consultant ? true : false,
        is_head: is_head ? true : false,
        is_tax_payer: is_tax_payer ? true : false,
        is_ssnit_payer: is_ssnit_payer ? true : false,
        exemptions: exemptions || [],

        //added
        entity_id: entity_id || "",
        gra_resident: gra_resident ? "resident" : "non_resident",
        gra_employment_type: gra_employment_type || "",
        gra_position: gra_position || "",
        staff_skill: staff_skill || "",
        staff_community: staff_community || "",
        resident_town: resident_town || "",
        marital_status: marital_status ? marital_status?.toLowerCase() : "",
        hometown: hometown || "",
        allowable_leave_days: allowable_leave_days,
        remaining_leave_days: remaining_leave_days,
        effective_date_of_last_promotion: formatDateForInput(
          effective_date_of_last_promotion
        ),
        is_auto_notch_update: is_auto_notch_update === false ? false : true,
        is_auto_status_update: is_auto_status_update === false ? false : true,
        is_payroll_eligible: is_payroll_eligible === false ? false : true,
        ...
        special_skills: special_skills || [{ name: "", proficiency_level: "" }],
      });
    }
  }, [employeeInfo]);
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      const { gra_resident, is_payroll_eligible } = formik.values;
      console.log("SUBMITTING PAYROLL ELIGIBLE:", is_payroll_eligible);
      const resident_format = gra_resident == "non_resident" ? false : true;

      await employeeUpdate({ ...formik.values, gra_resident: resident_format });
    },
  });
  ...
  const { data: passportImage, refetch: refetchImage } = useEmployeeImageQuery(
    {
      id: values.passport_image,
    },
    {
      refetchOnMountOrArgChange: true,
      skip: !values.passport_image || values.passport_image instanceof File,
    }
  );
  ...
  const { data: SignatureData, refetch: refetchSignature } =
    useEmployeeImageQuery(
      ...
      {
        refetchOnMountOrArgChange: true,
        skip: !values.signature || values.signature instanceof File,
      }
    );
  ...
  useEffect(() => {
    if (isSuccess) {
      refetchEmployee();
      if (values.passport_image && !(values.passport_image instanceof File)) {
        refetchImage();
      }
      if (values.signature && !(values.signature instanceof File)) {
        refetchSignature();
      }
      const message = `${data?.message}` || "The employee has been updated.";
      Swal.fire({
        title: message,
        icon: "success",
        timer: 5000,
  ...
  const passportImageUrl =
    values.passport_image instanceof File
      ...
      : passportImage?.url || "/images/user-avatar.png";

  const signatureUrl =
    values.signature instanceof File
      ? URL.createObjectURL(values.signature)
      : SignatureData?.url || "/images/user-avatar.png";
  ...
  return (
    <>
      <div className="pt-4 px-2">
        {!isDrawer && (
          <h1 className="text-[18px] mb-3 font-bold">Edit Employee</h1>
        )}

        {employeeInfo && (
          <div className="mt-8 bg-white flex justify-between px-5 py-4 mx-auto shadow-custom">
            <div>
              <button
              ...
              >
                ...
                <span className="flex flex-col items-center">
                  Prev Employee <span>{employeeInfo.prev_slug}</span>
                </span>
              </button>
            </div>
            <div>
              ...
              >
                <span className="flex flex-col items-center">
                  Next Employee <span>{employeeInfo.next_slug}</span>
                </span>
          ...
          <form onSubmit={handleSubmit}>
                    ...
                    <td className="w-full md:w-1/2 p-2 border-l-0">
                      <div>
                        ...
                        <AntdSelect
                          ...
                          className={`w-full text-[14px] bg-transparent rounded text-[#6c757d] outline-none mt-[4px]`}
                        />
                      </div>
                    </td>
                  </tr>
                    ...
                    <td className="w-full md:w-1/2 p-2 border-l-0">
                      <div>
                        ...
                        <AntdSelect
                          ...
                          onChange={(value) =>
                            setFieldValue("bank_branch_id", value)
                          }
                          filterOption={(input, option) =>
                            (option?.label as string)
                              ?.toLowerCase()
            ...
            <div className="overflow-x-auto mt-6 bg-white p-4">
              ...
              <table className="table-auto w-full bg-white">
                    ...
                    <td className="w-full md:w-1/2 p-2 border-l-0">
                      <div>
                        ...
                        rankList?.data?.find(
                          ...
                          <AntdSelect
                            disabled={values.is_auto_notch_update}
                            showSearch
                    ...
                    <td className="w-full md:w-1/2 p-2">
                      <div>
                        <label
                          ...
                          className={`${styles.label}`}
                        >
                          Hire Date/Date Of First Appointment
                        </label>
                        <input
                          type="date"
                          id="hire_start_date"
                ...
                <tbody className="divide-y hidden">
                  <tr className="flex flex-col md:flex-row ">
                    <td className="w-full md:w-1/2 p-2 border-l-0">
                      <div>
                        <label
                          htmlFor="remaining_leave_days"
                          className={`${styles.label}`}
                        >
                ...
                <tbody className="divide-y">
                  <tr className="flex flex-col md:flex-row  items-center">
                    ...
                    <td className="w-full md:w-1/2 p-2">
                      <div
                        className="mb-3"
                        hidden={values.wage_type === "established"}
                      >
                        <label
                          htmlFor="hourly_rate"
                          className={`${styles.label} text-black font-extrabold`}
                        >
                          Individual Hourly Override (Overrides Rank Rate)
                        </label>
                        <input
                          type="number"
                          id="hourly_rate"
                          ...
                          onChange={handleChange}
                          className={`${styles.form_input} border-blue-600 ring-2`}
                          placeholder="Ex: 200"
                        />
                        <p className="text-sm text-gray-500 mt-1">
                          If a value is entered here, this rate will be used for
                          payroll calculations instead of the hourly rate
                          assigned to the employee's rank. Leave blank to use
                          the rank's default rate. This applies to
                          hourly/clock-in employees only.
                        </p>
                      </div>
                      <div
                        className="mb-3"
                        hidden={values.wage_type === "clock_in"}
                      >
                        <label
                          htmlFor="basic_salary"
                          className={`${styles.label} text-black font-extrabold`}
                        >
                          Individual Monthly Salary Override (Overrides Rank
                          Salary)
                        </label>
                        <input
                          type="number"
                        ...
                        />
                        <p className="text-sm text-gray-500 mt-1">
                          If a value is entered here, this monthly salary will
                          be used for payroll calculations instead of the
                          monthly salary assigned to the employee's rank. Leave
                          blank to use the rank's default salary. This applies
                          to salaried/established employees only.
                        </p>
                      ...
                      <div>
                        <label
                          className={`${styles.label} font-extrabold text-[#ff5733]`}
                        >
                        ...
                        <AntdSelect
                          ...
                          style={{ width: "100%" }}
                          placeholder="Choose..."
                          value={formik.values.exemptions} // Bind to Formik's employees value
                          optionFilterProp="label"
                          filterOption={(input, option) =>
                            (option?.label as string)
                              .toLowerCase()
                          ...
                          onChange={(selectedOptions: any) => {
                            formik.setFieldValue("exemptions", selectedOptions);
                          }}
                        >
                          {taxReliefData &&
                            taxReliefData?.data?.map(
                              (relief: any, i: number) => (
                                <Option
                                  key={i}
                                  value={relief._id}
                                  label={`${relief.name}`} // Set label for search
                                >
                                  {`${relief.name} (${relief.amount})`}
                                </Option>
                              )
                            )}
                        </AntdSelect>
            ...
            <div className="overflow-x-auto mt-6 bg-white p-4 ">
              ...
              <div className="flex flex-wrap">
                <div className="w-full md:w-1/2 p-2">
                  ...
                  <br />
                  {values.academic_qualifications?.map(
                    (qualification, index) => (
                        ...
                        <section className="flex gap-2">
                          <div className="flex flex-col w-[45%]">
                            ...
                            <input
                              type="date"
                              name="start_date"
                              value={qualification.start_date?.substring(0, 10)}
                              className={`${styles.input}`}
                              onChange={(e) =>
                                handleQualificationChange(index, e)
                              }
                          ...
                          <div className="flex flex-col w-[45%]">
                            ...
                            <input
                              type="date"
                              name="end_date"
                              value={qualification.end_date?.substring(0, 10)}
                              className={`${styles.input}`}
                              onChange={(e) =>
                                handleQualificationChange(index, e)
                              }
                          ...
                          <input
                            ...
                            value={
                              qualification?.expires
                                ? qualification.end_date?.substring(0, 10) || ""
                                : ""
                            }
                            className={`${styles.input}`}
                            onChange={(e) =>
