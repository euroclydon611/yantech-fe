...
const Banks = () => {
  ...
  const hasBankCreateAccess = privileges?.includes("HR_BANK_CREATE");
  const hasBankEditAccess = privileges?.includes("HR_BANK_EDIT");
  const hasBankDeleteAccess = privileges?.includes("HR_BANK_DELETE");
  ...
  const {
    ...
    refetch,
  } = useBankListQuery({ page, limit, searchTerm, sortField, sortOrder });

  const handleChangePage = (page: any) => {
    setPage(page);
  };

  const handleDeleteButtonClick = async (bank: any) => {
  ...
  return (
    <>
      <AddBank open={add} onClose={() => setAdd(false)} refetch={refetch} />
      <EditBank open={edit} onClose={() => setEdit(false)} itemData={bank} refetch={refetch} />
      {loadOpen && (
        <CustomModal
          open={loadOpen}
          setOpen={setLoadOpen}
      ...
      <PageLayout
        title="Banks"
        subtitle="Financial institutions registered in the system"
        breadcrumbs={[{ label: "HR MGT" }, { label: "Banks" }]}
        actions={
          hasBankCreateAccess ? (
            <>
              <button className={`${styles.primary_button}`} onClick={() => setAdd(true)}>
                <LiaPlusSquareSolid size={16} />
                <span className="ml-1.5 font-medium">New Bank</span>
              </button>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4">
          ...
          <div className="table-wrapper max-h-[65vh] overflow-y-auto bg-white rounded-sm shadow-sm">
            <Table
              columns={[
                ...
                if (col.key === "action") {
                  return hasBankEditAccess || hasBankDeleteAccess;
                }
                return true;
              }) as any[]}
              dataSource={bankListData?.data || []}
              rowKey="_id"
              loading={false}
              pagination={false}
              size="small"
              locale={{ emptyText: "No banks found" }}
          ...
          <div className="flex mt-4 justify-end">
            <Pagination
              current={page}
              pageSize={limit}
              total={bankListData?.totalCount || 0}
