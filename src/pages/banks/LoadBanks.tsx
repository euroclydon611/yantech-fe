...
import { useLoadBanksDataMutation } from "../../redux/features/bank/bankApi";
...
const LoadBanks = ({ setOpen, refetch }: any) => {
  const [loadBanksData, { data, isSuccess, isLoading, error }] =
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      const failedMessage = failed
        ?.map(
          (item: any) =>
            `${item["Bank Name"] || "Unknown Bank"} - ${item["ERROR"] || item["error"] || "Unknown Error"}`
        )
        .join("<br><hr>");
