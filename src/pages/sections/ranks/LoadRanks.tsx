...
import { useLoadRanksDataMutation } from "../../../redux/features/sections/ranksApi";
...
const LoadRanks = ({ setOpen, refetch }: any) => {
  const [loadRanksData, { data, isSuccess, isLoading, error }] =
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      const failedMessage = failed
        ?.map(
          (item: any) =>
            `${item["Rank"] || item["Name"] || item["NAME"] || "Unknown Rank"} - ${item["ERROR"] || item["error"] || "Unknown Error"}`
        )
        .join("<br><hr>");
