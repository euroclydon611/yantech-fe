...
import { useRankStoreMutation } from "../../../redux/features/sections/ranksApi";
...
const AddRank = ({ open, onClose, refetch }: AddRankProps) => {
  ...
  const [rankStore, { data, isSuccess, isLoading, error }] =
  ...
  const formik = useFormik({
    initialValues: {
      ...
      basic_salary: "0",
      hourly_rate: "0",
      allowable_leave_days: "0",
    },
    validationSchema: schema,
    onSubmit: async ({ name, basic_salary, hourly_rate }) => {
      await rankStore({
        name,
  ...
  const notchTableColumns: any[] = [
    {
      ...
      render: (_text: unknown, _record: unknown, index: number) => {
        const notchKey = Object.keys(notches)[index];
        return notchKey;
      },
    },
    {
    ...
    //         size="small"
    //         placeholder="Select..."
    //         value={notches[notchKey].allowances_ids}
    //         onChange={(value) =>
    //           handleNotchFieldChange(notchKey, "allowances_ids", value)
    //         }
    //         style={{ width: "100%" }}
    //       >
    //         {allowancesList &&
    ...
    //         size="small"
    //         placeholder="Select..."
    //         value={notches[notchKey].deductions_ids}
    //         onChange={(value) =>
    //           handleNotchFieldChange(notchKey, "deductions_ids", value)
    //         }
    //         style={{ width: "100%" }}
    //       >
    //         {allowancesList &&
  ...
  const notchDataSource = Object.keys(notches).map((key, index) => ({
    key: index,
    notch: key,
  }));
      ...
      <div className="fade-in">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <div className="!hidden">
            <label htmlFor="basic_salary" className={`${styles.label}`}>
              Basic Salary
            </label>
            <input
              type="number"
          ...
          {/* 
        <div>
          <label htmlFor="allowable_leave_days" className={`${styles.label}`}>
