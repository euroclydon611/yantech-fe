...
const EditEntity = ({ open, onClose, itemData, refetch }: EditEntityProps) => {
  const [reporting_entity_id, setSetReporting_entity_id] = useState(
    itemData?.reporting_entity_id || ""
  );

  const [EntityUpdate, { data, isSuccess, error, isLoading }] =
    useEntityUpdateMutation();
  ...
  const formik = useFormik({
    initialValues: {
      ...
      designation: itemData?.designation as string,
      branch_code: itemData?.branch_code as string,
      officeLocation: (itemData?.officeLocation as string) || "",
      from: parseInt(itemData?.from) as number,
      to: parseInt(itemData?.to) as number,
      head_start_date: head_start_date,
      head_end_date: head_end_date,
    },
    enableReinitialize: true,
    validationSchema: schema,
    onSubmit: async ({ name, designation, branch_code, officeLocation, from, to }) => {
      if (
        (!reporting_entity_id || reporting_entity_id.trim() === "") &&
        values.designation !== "ceo office"
      ) {
        Swal.fire({
          icon: "warning",
          title: "Missing Field",
  ...
  useEffect(() => {
    if (open) {
      setSetReporting_entity_id(itemData?.reporting_entity_id || "");
    } else {
      formik.resetForm();
    }
  }, [open, itemData]);
  ...
  useEffect(() => {
    if (isSuccess) {
      const message = `${data?.message}` || "The entity has been updated.";
      Swal.fire({
        title: message,
        icon: "success",
        timer: 5000,
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div>
            ...
            <select
              ...
              className={`${styles.input} disabled:bg-slate-50`}
              // disabled
            >
              <option value="">- - - - - -</option>
          ...
          <div className="hidden">
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                className={`${styles.input}`}
                placeholder="Enter COA code Start Value"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                className={`${styles.input}`}
                placeholder="Enter COA code End Value"
              />
