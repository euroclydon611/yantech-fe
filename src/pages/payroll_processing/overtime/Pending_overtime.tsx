...
const Pending_overtime = () => {
  PageTitle("Pending Overtime | Payroll");
  ...
  return (
    <>
      ...
      <PageLayout
        ...
        subtitle="Review and manage pending overtime requests"
      ...
      <div className="bg-white mt-5 p-4">
          ...
          <table className={`${styles.table}`}>
            <thead className={`${styles.thead} text-xs`}>
              <tr>
                ...
                <th className={`${styles.wide_tb_th2} !text-center`}>Action</th>
              </tr>
            </thead>
            <tbody className="text-xs">
              {pendingList?.data?.overtime_data &&
              pendingList?.data?.overtime_data.length > 0 ? (
                pendingList?.data?.overtime_data?.map(
                  (overtime: any, i: any) => (
                    <tr
                      key={i}
                      className={`${
                    ...
                    >
                      ...
                      <td className={`${styles.td}`}>
                        <span className="text-yellow-600 font-semibold">
                          {capitalizeFirstLetter(overtime?.status) || "Pending"}
                        </span>
                      </td>
        ...
        <div className="flex  mt-4 justify-end">
          <Pagination
            current={page}
            pageSize={limit}
            total={(pendingList?.data && pendingList?.data.totalCount) || 1}
...
export default Pending_overtime;
