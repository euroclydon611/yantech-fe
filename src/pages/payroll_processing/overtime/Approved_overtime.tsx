...
import AdminRequestOvertime from "./AdminRequestOvertime";
...
const Approved_overtime = () => {
  PageTitle("Approved Overtime | Payroll");
  ...
  const { data: batchList, isLoading: batchLoading } =
  ...
  return (
    <>
      {add && hasOvertimeCreateAccess && (
        <CustomModal
          open={add}
          setOpen={setAdd}
          Component={AdminRequestOvertime}
          refetch={refetch}
        />
      )}
      ...
      <PageLayout
        ...
        subtitle="View and manage approved overtime requests"
      ...
      <section className="pt-6 px-3">
        <h3 className="text-xl font-semibold text-gray-800">
          Approved Overtime Applications
        </h3>
      </section>
      ...
      <div className="bg-white mt-5 p-4">
        <div className="space-y-4">
          <div className="flex justify-between mb-2 max-sm:flex-wrap gap-2">
            ...
            {hasOvertimeCreateAccess && (
              <button
                className={`${styles.primary_button}`}
                onClick={() => setAdd(true)}
                title="Add overtime for employees"
              >
                <LiaPlusSquareSolid size={18} />
                <span className="ml-2 max-md:hidden font-medium">New</span>
              </button>
            )}
          </div>
          ...
          <table className={`${styles.table}`}>
            <thead className={`${styles.thead} text-xs`}>
              <tr>
                ...
                <th className={`${styles.wide_tb_th2} border`}>Staff ID</th>
                ...
                <th className={`${styles.wide_tb_th2} border`}>Status</th>
                <th className={`${styles.wide_tb_th2} border`}>
                  Decision Note
                </th>
                <th className={`${styles.wide_tb_th2} border`}>Action By</th>
                <th className={`${styles.wide_tb_th2} border`}>Role</th>
                <th className={`${styles.wide_tb_th2} border`}>Action</th>
              </tr>
            </thead>
            <tbody className="text-xs">
              {approvedList.data?.overtime_data &&
              approvedList?.data?.overtime_data.length > 0 ? (
                approvedList?.data?.overtime_data?.map(
                  (overtime: any, i: any) => (
                    <tr
                      key={i}
                      className={`${
                    ...
                    >
                      ...
                      <td className={`${styles.td}`}>
                        {overtime?.overtime_date &&
                          formatDate2(overtime?.overtime_date)}
                      </td>
                      <td className={`${styles.td}`}>
                        {overtime?.created_at &&
                          formatDate2(overtime?.created_at)}
                      </td>
                      <td className={`${styles.td}`}>
        ...
        <div className="flex  mt-4 justify-end">
          <Pagination
            current={page}
            pageSize={limit}
            total={(approvedList?.data && approvedList.data.totalCount) || 1}
...
export default Approved_overtime;
