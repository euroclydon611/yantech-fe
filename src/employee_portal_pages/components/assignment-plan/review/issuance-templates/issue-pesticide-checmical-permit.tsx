...
interface IssuePesticideIndustrialChemicalPermitProps {
  applicationData: any;
  task?: any;
...
const IssuePesticideIndustrialChemicalPermit: React.FC<
  IssuePesticideIndustrialChemicalPermitProps
> = ({ applicationData, task, isSubmitting, onSubmit, onSave }) => {
  const [form] = Form.useForm();
  const [sectionsModified, setSectionsModified] = React.useState(false);

  const { _id,clientId, clientDetails, answers } = applicationData;
  const { permitDetails } = answers || {};

  const getPermitTitle = () => {
    const combinedType = `${permitDetails?.permitType}_${permitDetails?.permitCategory}`;
    switch (combinedType) {
      case "import_pesticide":
        return "Pesticide Import Permit";
      case "export_pesticide":
        return "Pesticide Export Permit";
      case "import_industrial_chemical":
        return "Industrial & Consumer Chemical Import Permit";
      case "export_industrial_chemical":
        return "Industrial & Consumer Chemical Export Permit";
      default:
  ...
  const getCompanyName = () => {
    ...
    const client = clientDetails || clientId || applicationData?.client;
  ...
  const buildPayload = async () => {
    ...
    const data = {
      permitApplicationId: _id,
      permitId: issuedPermit?.data?._id,
      applicationId: _id,
      clientId: clientId?._id,
      permitType:
        values.permitType ||
        `${permitDetails?.permitType}_${permitDetails?.permitCategory}`,
...
export default IssuePesticideIndustrialChemicalPermit;
