...
import { useRejectedLeavesQuery } from "../../../redux/features/employee-portal-api/leaveApi";
...
export default function RejectedLeaves({ employee }: LeaveApprovalProps) {
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(25);
  const [selectedLeave, setSelectedLeave] = useState<any>(null);
  const [drawerOpen, setDrawerOpen] = useState(false);

  const { data: rejectedList, isLoading: pendingLoading } =
  ...
  const rejectedLeaves = rejectedList?.data?.rejected_leaves || [];
    ...
    },
    {
      ...
      render: (_, record) => (
        <Text style={{ fontSize: '13px' }}>
          {formatDate2(record.created_at || (record as any).createdAt)}
        </Text>
      ),
    },
    {
    ...
    <div className="bg-white">
      ...
      <Table 
        ...
        pagination={{ 
          pageSize: 10,
          current: page,
          total: rejectedList?.data?.totalCount,
          onChange: (p) => setPage(p)
        }}
        locale={{
          emptyText: <Empty description="No rejected leave applications found." image={Empty.PRESENTED_IMAGE_SIMPLE} />
        }}
        scroll={{ x: 800 }}
      />
    </div>
