...
import { useApprovedLeavesQuery } from "../../../redux/features/employee-portal-api/leaveApi";
...
export default function ApprovedLeaves({ employee }: LeaveApprovalProps) {
  const [page, setPage] = useState(1);
  const [limit] = useState(25);
  const [selectedLeave, setSelectedLeave] = useState<any>(null);
  const [drawerOpen, setDrawerOpen] = useState(false);

  const { data: approvedList, isLoading: pendingLoading } =
  ...
  const approvedLeaves = approvedList?.data?.approved_leaves || [];
    ...
    <div className="bg-white">
      ...
      <Table 
        ...
        pagination={{ 
          pageSize: 10,
          current: page,
          total: approvedList?.data?.totalCount,
          onChange: (p) => setPage(p)
        }}
        locale={{
          emptyText: <Empty description="No approved leave applications found." image={Empty.PRESENTED_IMAGE_SIMPLE} />
        }}
        scroll={{ x: 800 }}
      />
    </div>
