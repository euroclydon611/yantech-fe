...
export const EnvironmentalUpstreamPreview: React.FC<
...
> = ({ application }) => {
  ...
  const upstreamLocation = energyData?.upstreamLocation;
  ...
  const getLocationTypeLabel = (type: string) => {
    const types: Record<string, string> = {
      onshore: "Onshore (Land-based)",
      offshore: "Offshore (Marine-based)",
    };
    return types[type] || type;
  };

  const getNatureOfApplicationLabel = (nature: string, other?: string) => {
  ...
  const getMainElementLabel = (element: string, other?: string) => {
    ...
    <div className={styles.reviewPage}>
      ...
      <div className={styles.gridContainer}>
        ...
        <Block title="3. Undertaking Information" className={styles.fullWidth}>
          <Detail
            label="Upstream Operation Location:"
            value={getLocationTypeLabel(upstreamLocation)}
          />
          <Detail
            label="Upstream Operation Type:"
          ...
          <Detail
            ...
            value={undertakingInfo?.shortDescription || "N/A"}
          />
        </Block>
        ...
        <Block title="4. Undertaking Location" className={styles.fullWidth}>
          <div className="mb-4">
            ...
            <Detail
              ...
              value={undertakingLocation?.mainUndertakingLocation || "N/A"}
            />
            <Detail
              label="Quadrant:"
              value={undertakingLocation?.quadrant || "N/A"}
            />
            <Detail
              label="Block Number:"
              value={undertakingLocation?.blockNumber || "N/A"}
            />
            {undertakingLocation?.blockSuffix && (
              <Detail
                label="Block Suffix:"
                value={undertakingLocation.blockSuffix || "N/A"}
              />
            )}
            <Detail
              label="Undertaking Location:"
              value={undertakingLocation?.undertakingLocation || "N/A"}
            />
            <Detail
              label="Latitude:"
              value={undertakingLocation?.latitude || "N/A"}
            />
            <Detail
              label="Longitude:"
              value={undertakingLocation?.longitude || "N/A"}
            />
          </div>
          ...
          <div className="mb-4">
            ...
            <Detail
              ...
              value={
                undertakingLocation?.distanceToCoastline
                  ? `${undertakingLocation.distanceToCoastline} km`
                  : "N/A"
              }
            />
            <Detail
              label="Which Coast:"
              value={normalizeText(undertakingLocation?.whichCoast) || "N/A"}
            />
            <Detail
              label="Distance to Transboundary:"
              value={
                undertakingLocation?.distanceToTransboundary
                  ? `${undertakingLocation.distanceToTransboundary} km`
                  : "N/A"
              }
            />
            <Detail
              label="Which Line:"
              value={undertakingLocation?.whichLine || "N/A"}
            />
          </div>
...
export default EnvironmentalUpstreamPreview;
