...
interface InvoiceStatisticsModalProps {
  open: boolean;
  onClose: () => void;
}
...
const InvoiceStatisticsModal: React.FC<InvoiceStatisticsModalProps> = ({
  open,
  onClose,
}) => {
  ...
  const { data: statsResponse, isLoading } = useFetchInvoicesStatisticsQuery(
    {
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <div className="text-[10px] text-gray-400 font-normal leading-tight uppercase tracking-widest">
              Entity-wide financial overview
            </div>
          </div>
        </div>
      }
                ...
                <ResponsiveContainer>
                  <BarChart data={invoiceStats.monthlyTrends} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
                    ...
                    <RechartsTooltip
                      ...
                      formatter={(value: number, name: string) => [`GH₵ ${formatNumber(value)}`, name]}
                ...
                columns={[
                  {
                    ...
                    key: "departmentName",
                    render: (text) => <Text strong className="text-xs text-gray-800">{text}</Text>,
                  },
                  {
                  ...
                  },
                  {
                    ...
                    width: "18%",
                    align: "right",
                    render: (amount) => <span className="text-xs font-bold text-amber-600">{formatNumber(amount)}</span>,
                  },
                  {
                    ...
                    width: "18%",
                    align: "right",
                    render: (amount) => <span className="text-xs font-bold text-green-600">{formatNumber(amount)}</span>,
                  },
                  {
