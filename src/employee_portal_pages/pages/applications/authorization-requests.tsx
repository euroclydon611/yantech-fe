...
import { formatDate2, normalizeText } from "@/utils/helperFunction";
import {
  getStatusBadge,
  statusOptions,
  authorizationRequestTypes,
...
import TransferAuthorizationRequestModal from "@/employee_portal_pages/components/application/transfer-authorization-request-modal";
...
const AuthorizationRequests = () => {
  PageTitle("Authorization Requests | EPA Ghana");

  const { employee } = useSelector((state: RootState) => state.employee_auth);
  const [fontSizeKey, setFontSizeKey] = useState<string>(
    () => localStorage.getItem("auth-req-font-size") || "base"
  );
  const zoom = AUTH_FONT_SIZES.find((f) => f.key === fontSizeKey)?.zoom ?? 1;
  const handleFontSize = (key: string) => { setFontSizeKey(key); localStorage.setItem("auth-req-font-size", key); };
  ...
  const filterOptions = useMemo(
    () => ({
      ...
      permitTypeOptions: [
        { label: "All", value: "all" },
        ...authorizationRequestTypes.map((type: string) => ({
          label: type
            .replace(/_/g, " ")
            .replace(/\b\w/g, (l) => l.toUpperCase()),
          value: type,
    ...
    () => [
      ...
      },
      {
        title: (
          <div
            className="flex items-center cursor-pointer hover:text-blue-600 transition-colors text-xs"
            onClick={() => handleSort("authorizationType")}
          >
            <span>Type</span>
        ...
        width: 180,
        sorter: true,
        sortOrder: sortField === "authorizationType" ? (sortOrder === "asc" ? "ascend" : "descend") : null,
        render: (authorizationType) => (
          <span className="font-mono text-[10px] font-medium">
            {normalizeText(authorizationType) || "N/A"}
          </span>
        ),
      },
      {
  ...
  return (
    <>
      ...
      {transferModalOpen && (
        <TransferAuthorizationRequestModal
      ...
      <StatisticsModal
        ...
        title="Authorization Request Statistics"
      />
      ...
      <div className="auth-page-root flex flex-col" style={{ zoom }}>
        ...
        <div className="px-4 pt-3 pb-1 border-b border-gray-100 bg-white">
          <Breadcrumb
            items={[
              ...
              { title: "Licensing MGT" },
              { title: <span className="text-green-700 font-medium">Authorization Requests</span> },
            ]}
            className="text-xs"
          />
        </div>
        ...
        <div className="px-4 py-2 bg-white border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-sm font-semibold text-gray-800 leading-tight truncate">Authorization Requests</h1>
              <p className="text-[11px] text-gray-500 leading-tight">
                {(employee as any)?.entity?.name || "All Entities"} &mdash; Manage &amp; review authorization requests
              </p>
            </div>
          </div>
        ...
        <div className="px-4 py-2 bg-gray-50 border-b border-gray-200">
          <Row gutter={[8, 8]} align="middle">
            ...
            <Col xs={12} sm={6} md={5} lg={4} xl={4}>
              <Select value={permitType} onChange={(v) => handleFilterChange("permitType", v)} className="w-full" size="small" placeholder="Auth Type" options={filterOptions.permitTypeOptions} />
            </Col>
            <Col xs={24} sm={14} md={8} lg={6} xl={6}>
              <RangePicker size="small" className="w-full" value={dateRange} onChange={handleRangeChange} placeholder={["Submitted from", "Submitted to"]} />
            </Col>
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              ...
              {permitType !== "all" && <Tag closable onClose={() => handleFilterChange("permitType", "all")} color="purple" className="text-[10px] leading-tight m-0">Type: {permitType.replace(/_/g, " ")}</Tag>}
              ...
              {startDate !== "" && <Tag closable onClose={() => handleFilterChange("dateRange", [null, null])} color="orange" className="text-[10px] leading-tight m-0">{formatDate2(startDate)} → {formatDate2(endDate)}</Tag>}
            </div>
          )}
        </div>
        ...
        <div className="px-4 py-2 border-t border-gray-200 bg-white flex flex-col sm:flex-row items-center justify-between gap-2">
          ...
          {paginationInfo && paginationInfo.total > 0 && (
            <Pagination current={page} pageSize={limit} total={paginationInfo.total}
              ...
              showSizeChanger showQuickJumper size="small" className="auth-pagination" />
          )}
        </div>
      </div>
      <style>{`
        .auth-page-root { min-height: calc(100vh - 48px); display: flex; flex-direction: column; }
        .auth-app-table .ant-table-thead > tr > th { background: #f1f5f9; font-weight: 600; font-size: 11px; color: #374151; border-bottom: 2px solid #e2e8f0; white-space: nowrap; }
        .auth-app-table .ant-table-tbody > tr > td { border-bottom: 1px solid #f1f5f9; font-size: 11px; }
        .auth-app-table .ant-table-tbody > tr:hover > td { background-color: #f0fdf4 !important; }
        .auth-app-table .ant-table-body { overflow-y: auto !important; }
        .auth-app-table .ant-table-cell-fix-right { background: #fff; }
        .auth-app-table tr.bg-gray-50\/60 .ant-table-cell-fix-right { background: #f9fafb; }
        .auth-app-table .ant-table-thead .ant-table-cell-fix-right { background: #f1f5f9 !important; }
        .auth-pagination .ant-pagination-item-active { border-color: #15803d; background-color: #15803d; }
        .auth-pagination .ant-pagination-item-active a { color: #fff; }
        .auth-pagination .ant-pagination-item-active:hover { border-color: #166534; background-color: #166534; }
