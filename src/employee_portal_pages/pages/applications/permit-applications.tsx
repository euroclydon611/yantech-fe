...
import { useFetchApplicationsForHeadQuery } from "@/redux/features/employee-portal-api/application/application";
...
import { formatDate2, formatDate4, normalizeText } from "@/utils/helperFunction";
import {
  getStatusBadge,
  statusOptions,
  permitTypes,
...
import TransferPermitApplicationModal from "@/employee_portal_pages/components/application/transfer-permit-application-modal";
...
import StatisticsModal from "@/employee_portal_pages/components/application/statistics-modal";
...
const PermitApplications = () => {
  ...
  const [fontSizeKey, setFontSizeKey] = useState<string>(
    () => localStorage.getItem("permit-app-font-size") || "base"
  );
  const zoom = PERMIT_FONT_SIZES.find((f) => f.key === fontSizeKey)?.zoom ?? 1;
  const handleFontSize = (key: string) => { setFontSizeKey(key); localStorage.setItem("permit-app-font-size", key); };

  const ownEntityId = (employee as any)?.entity?._id || (employee as any)?.entity_id;
  const isCEO = (employee as any)?.entity?.designation?.toLowerCase() === "ceo office" ||
  ...
  const { data: allEntitiesResponse } = useEntityFullListQuery(
    {},
    { skip: !isCEO && currentView !== "transferred" }
  );
  const { data: reporteesResponse } = useEntityReporteesQuery(
    { page: 1, limit: 200, searchTerm: "", entity_id: ownEntityId },
    { skip: !ownEntityId || isCEO || currentView === "transferred" }
  );
  const reporteeEntities = reporteesResponse?.data || [];
  const ownEntityOption = ownEntityId && (employee as any)?.entity?.name
    ? [{ _id: ownEntityId, name: (employee as any)?.entity?.name, id: ownEntityId }]
    : [];
  const entities = (isCEO || currentView === "transferred")
  ...
  const [transferDateRange, setTransferDateRange] = useState<[Dayjs | null, Dayjs | null]>([null, null]);

  const startDate = dateRange?.[0]?.format("YYYY-MM-DD") || "";
  const endDate = dateRange?.[1]?.format("YYYY-MM-DD") || "";
  const transferStartDate = transferDateRange?.[0]?.format("YYYY-MM-DD") || "";
  const transferEndDate = transferDateRange?.[1]?.format("YYYY-MM-DD") || "";
  ...
  const stats = applicationsResponse?.stats || {};
  ...
  const clearAllFilters = useCallback(() => {
    ...
    setSortOrder("desc");
    setDateRange(null);
    setTransferDateRange([null, null]);
    setPage(1);
  }, []);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_PUBLIC_SERVER_URI_HR}/revenue/application/permit-applications/export?${params.toString()}`,
        { withCredentials: true, responseType: "blob" }
      );
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", `Permit_Applications_${Date.now()}.xlsx`);
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
    } catch {
      Swal.fire({ title: "Export Failed", text: "Unable to export applications.", icon: "error", confirmButtonColor: "#727cf5" });
    } finally {
  ...
  const tableColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        width: 150,
        render: (_, record: any) => (
          <Tag className="text-[11px] capitalize" color="">
            {normalizeText(record?.answers?.permitDetails?.applicationType)}
          </Tag>
        ),
      },
      {
        title: (
          ...
          >
            ...
            <SortIndicator field="permitType" />
          </div>
        ),
        ...
        width: 180,
        sorter: true,
        sortOrder: sortField === "permitType" ? (sortOrder === "asc" ? "ascend" : "descend") : null,
        render: (permitType) => (
          <span className="font-mono text-[10px] font-medium">
            {normalizeText(permitType) || "N/A"}
          </span>
        ),
      },
      {
        title: <div className="text-xs">Application Details</div>,
        key: "details",
        width: 300,
        render: (_, app) => (
          <div className="space-y-1">
            <Tooltip title={app.title}>
              <p
                className="font-medium text-gray-800 text-[10px] line-clamp-2"
                title={app.title}
              >
      ...
      {
        title: <div className="text-xs text-center">Actions</div>,
        key: "actions",
        width: 120,
        align: "center",
        fixed: "right",
    ...
    <>
      {viewModalOpen && (
        <ApplicationPreview
          ...
          type={"permit"}
          canTransfer={currentView === "current" && (employee?.is_head || employee?.permissions?.includes("transfer_application"))}
          onTransfer={() => {
      ...
      {transferModalOpen && (
        <TransferPermitApplicationModal
      ...
      <StatisticsModal
        ...
        stats={applicationsResponse?.stats || {}}
        title="Permit Application Statistics"
      />
      ...
      <div className="permit-page-root flex flex-col" style={{ zoom }}>
        ...
        <div className="px-4 pt-3 pb-1 border-b border-gray-100 bg-white">
          <Breadcrumb
            items={[
              ...
              { title: "Permit MGT" },
              { title: <span className="text-green-700 font-medium">Applications</span> },
            ]}
            className="text-xs"
          />
        </div>
        ...
        <div className="px-4 py-2 bg-white border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-sm font-semibold text-gray-800 leading-tight truncate">Permit Applications</h1>
              <p className="text-[11px] text-gray-500 leading-tight">
                {(employee as any)?.entity?.name || "All Entities"} &mdash; Manage &amp; review applications
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            <div className="flex items-center border border-gray-200 rounded overflow-hidden">
              {PERMIT_FONT_SIZES.map((f) => (
                <button
                  key={f.key}
                  onClick={() => handleFontSize(f.key)}
                  title={f.key === "sm" ? "Small" : f.key === "base" ? "Normal" : "Large"}
                  className={`px-2 py-[3px] text-[10px] font-bold border-r last:border-r-0 border-gray-200 transition-colors ${
            ...
            <Tooltip title="Statistics">
              <Button size="small" icon={<BarChartOutlined />} onClick={() => setStatsModalOpen(true)} className="border-blue-300 text-blue-600 hover:!bg-blue-50" />
            </Tooltip>
            <Tooltip title="Export to Excel">
              <Button size="small" icon={<FileExcelOutlined />} loading={isExporting} onClick={handleExport} className="border-green-600 text-green-700 hover:!bg-green-50" />
            </Tooltip>
        ...
        <div className="px-4 py-2 bg-gray-50 border-b border-gray-200">
          <Row gutter={[8, 8]} align="middle">
            ...
            <Col xs={12} sm={6} md={5} lg={4} xl={4}>
              <Select
                value={permitType}
                onChange={(v) => handleFilterChange("permitType", v)}
                className="w-full"
                size="small"
            ...
            <Col xs={24} sm={14} md={8} lg={6} xl={6}>
              {currentView === "transferred" ? (
                <RangePicker
                  size="small"
                  className="w-full"
                  value={transferDateRange}
                  onChange={(dates) => setTransferDateRange(dates ?? [null, null])}
                  placeholder={["Transfer from", "Transfer to"]}
                />
              ) : (
                <RangePicker
                  size="small"
            ...
            <Col xs={24} sm={10} md={24} lg={5} xl={6}>
              <Select
                showSearch
          ...
          {activeFiltersCount > 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {searchTerm && (
                <Tag closable onClose={() => handleFilterChange("search", "")} color="blue" className="text-[10px] leading-tight m-0">
                  Search: "{searchTerm}"
                </Tag>
              )}
              {status !== "all" && (
                <Tag closable onClose={() => handleFilterChange("status", "all")} color="green" className="text-[10px] leading-tight m-0">
                  Status: {status.replace(/_/g, " ")}
                </Tag>
              )}
              {permitType !== "all" && (
                <Tag closable onClose={() => handleFilterChange("permitType", "all")} color="purple" className="text-[10px] leading-tight m-0">
                  Type: {permitType.replace(/_/g, " ")}
                </Tag>
              )}
              {assigningEntity !== "all" && (
                <Tag closable onClose={() => handleFilterChange("assigningEntity", "all")} color="cyan" className="text-[10px] leading-tight m-0">
                  Entity: {(entities as any[]).find((e: any) => (e.id || e._id) === assigningEntity)?.name || assigningEntity}
                </Tag>
              )}
              {startDate !== "" && (
                    ...
                    description={
                      <div className="py-4">
                        <p className="text-gray-500 text-sm">
                          {activeFiltersCount > 0 ? "No applications match the current filters." : "No applications found."}
                        </p>
                        {activeFiltersCount > 0 && (
                          <Button type="link" size="small" onClick={clearAllFilters}>Clear filters</Button>
                        )}
                      </div>
                    }
                  />
      ...
      <style>{`
        ...
        .permit-app-table .ant-table-thead > tr > th {
          background: #f1f5f9;
          font-weight: 600;
          font-size: 11px;
          color: #374151;
        ...
        .permit-app-table .ant-table-tbody > tr > td {
          border-bottom: 1px solid #f1f5f9;
          font-size: 11px;
        }
        .permit-app-table .ant-table-tbody > tr:hover > td {
          background-color: #f0fdf4 !important;
        }
        .permit-app-table .ant-table-body {
          overflow-y: auto !important;
        }
        .permit-app-table .ant-table-cell-fix-right {
          background: #fff;
        }
        .permit-app-table .ant-table-cell-fix-right-first::after {
          box-shadow: inset -6px 0 8px -4px rgba(0,0,0,.12);
        }
        .permit-app-table tr.bg-gray-50\/60 .ant-table-cell-fix-right,
        .permit-app-table tr.bg-gray-50\/60 .ant-table-cell-fix-left {
          background: #f9fafb;
        }
        .permit-app-table .ant-table-thead .ant-table-cell-fix-right {
          background: #f1f5f9 !important;
        }
        .permit-pagination .ant-pagination-item-active {
          border-color: #15803d;
          background-color: #15803d;
        }
        .permit-pagination .ant-pagination-item-active a { color: #fff; }
        .permit-pagination .ant-pagination-item-active:hover {
          border-color: #166534;
