...
export default function StaffPage() {
  ...
  return (
    <div className="staff-page-root flex flex-col">
      ...
      <div className="px-4 pt-3 pb-1 border-b border-gray-100 bg-white">
        <Breadcrumb
          items={[
            ...
            { title: "Personal" },
            { title: <span className="text-green-700 font-medium">Team Members</span> },
          ]}
          className="text-xs"
        />
      </div>
      ...
      <div className="px-4 py-2 bg-white border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-semibold text-gray-800 leading-tight truncate">Team Members</h1>
            <p className="text-[11px] text-gray-500 leading-tight">
              {(employee as any)?.entity?.name || "Your entity"} &mdash; Manage staff and permissions
            </p>
          </div>
        </div>
      ...
      <style>{`
        .staff-page-root { min-height: calc(100vh - 48px); display: flex; flex-direction: column; }
      `}</style>
    </div>
  );
}
