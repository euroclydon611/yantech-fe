...
import { useEmployeeVerifyOTPMutation } from "../redux/features/employee-portal-api/authApi";
...
const Verify_otp = () => {
  PageTitle("Verify OTP | Employee Activation");
  ...
  const formik = useFormik({
    initialValues: { staff_id: activation_staff_id || "", otp: "" },
    validationSchema: schema,
    onSubmit: async ({ staff_id, otp }) => {
      await verifyOTP({ staff_id, otp });
    },
  });
      ...
      <div className="relative z-10 flex justify-center px-4 pb-2 animate-fadeIn">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-green-400 text-[11px] font-semibold">Verify OTP</span>
          </div>
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center px-4 sm:px-8 py-6">
        <div className="w-full max-w-md animate-slideUp">
          <div className="mb-7">
            ...
            <p className="text-slate-400 text-sm mt-1.5">Enter the verification code sent to your official email</p>
          </div>
          ...
          <div className="bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-slate-700 p-6 sm:p-8 shadow-2xl shadow-black/50">
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="pt-4 border-t border-slate-700 flex flex-col sm:flex-row justify-between items-center gap-3">
                <Link
                  to="/employee/activate-account"
                  className="text-green-400 hover:text-green-300 text-sm font-medium transition-colors duration-200 flex items-center gap-1.5 group"
                >
                ...
                <Link
                  to="/employee"
                  className="text-slate-500 hover:text-slate-400 text-sm font-medium transition-colors duration-200"
                >
                  Back to Login
                </Link>
              </div>
          ...
          <div className="mt-4 p-4 rounded-xl border border-slate-700 bg-slate-800/50 animate-fadeIn">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-slate-300 text-xs font-bold tracking-wider uppercase mb-1">OTP Verification</p>
                <p className="text-slate-400 text-xs leading-relaxed">
                  Check your official email for a 6-digit verification code. The code expires in 10 minutes.
                </p>
              </div>
            </div>
          </div>
