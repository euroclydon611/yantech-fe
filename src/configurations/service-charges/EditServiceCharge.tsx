...
import { useUpdateServiceChargeMutation } from "../../redux/features/configurations/service_charges";
...
const EditServiceCharge = ({ setOpen, itemData, refetch }: any) => {
  const [updateServiceCharge, { data, isSuccess, error, isLoading }] =
    useUpdateServiceChargeMutation();

  const formik = useFormik({
    initialValues: {
      min_amount: itemData?.min_amount || 0,
      max_amount: itemData?.max_amount || null,
      charge: itemData?.charge || 0,
      is_cap: itemData?.is_cap || false,
    },
    validationSchema: schema,
    onSubmit: async (values) => {
      await updateServiceCharge({ _id: itemData._id, ...values });
    },
  });
