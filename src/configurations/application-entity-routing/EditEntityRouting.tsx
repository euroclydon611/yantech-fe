...
import { useUpdateEntityRoutingMutation } from "../../redux/features/configurations/entity_routing";
...
const EditEntityRouting = ({ setOpen, itemData, refetch }: any) => {
  const [updateEntityRouting, { data, isSuccess, error, isLoading }] =
  ...
  const formik = useFormik({
    initialValues: {
      applicationType: itemData?.applicationType || "",
      applicationCategory: itemData?.applicationCategory || "",
      permitType: itemData?.permitType || "",
      category: itemData?.category || "",
      subcategory: itemData?.subcategory || "",
      sector: itemData?.sector || "",
      entityPattern: itemData?.entityPattern || "",
      entityRegex: itemData?.entityRegex || "",
      active: itemData?.active ?? true,
    },
    validationSchema: schema,
    onSubmit: async (values) => {
      await updateEntityRouting({ _id: itemData._id, ...values });
    },
  });
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          {errors.applicationCategory && touched.applicationCategory && (
            <span className="text-red-500 pt-2 block fade-in">
              {errors.applicationCategory as string}
            </span>
          )}
        </div>
        ...
        <div>
          ...
          {errors.entityPattern && touched.entityPattern && (
            <span className="text-red-500 pt-2 block fade-in">
              {errors.entityPattern as string}
            </span>
          )}
        </div>
...
export default EditEntityRouting;
