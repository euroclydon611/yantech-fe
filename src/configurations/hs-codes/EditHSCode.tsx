...
import { useUpdateHsCodeMutation } from "@/redux/features/configurations/hscodes";
...
const EditHSCode = ({ setOpen, itemData, refetch }: any) => {
  PageTitle("Edit Code");

  const [updateCode, { data, isSuccess, isLoading, error }] =
  ...
  const formik = useFormik({
    initialValues: {
      ...
      assessments: itemData?.assessmentTypes?.map((item) => item._id) ?? [],
    },
    validationSchema: schema,
    onSubmit: async (values) => {
      await updateCode({ ...values, id: itemData._id });
    },
  });
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white rounded-lg shadow-md">
      <h1 className="text-2xl font-semibold mb-4">Edit HS Code</h1>
