...
import { useLoadHsCodesMutation } from "../../redux/features/configurations/hscodes";
...
const LoadCodes = ({ setOpen, refetch }: any) => {
  const [loadHSCodes, { data, isSuccess, isLoading, error }] =
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      const { failedRecords, totalRecords, insertedRecords } = data;

      const outcome = `
      Total Rows: ${totalRecords}<br><br>
      Inserted: ${insertedRecords}<br><br>
      <strong>Failed Records: </strong>${failedRecords}
    `;
