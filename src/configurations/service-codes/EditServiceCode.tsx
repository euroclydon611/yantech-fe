...
import { useUpdateServiceCodeMutation } from "../../redux/features/configurations/service_codes";
...
const EditServiceCode = ({ setOpen, itemData, refetch }: any) => {
  const [updateServiceCode, { data, isSuccess, error, isLoading }] =
    useUpdateServiceCodeMutation();

  const formik = useFormik({
    initialValues: {
      name: itemData?.name as string,
      service_code: itemData?.service_code as string,
      status: (itemData?.status as string) || "active",
    },
    validationSchema: schema,
    onSubmit: async ({ name, service_code, status }) => {
      await updateServiceCode({ _id: itemData._id, name, service_code, status });
    },
  });
